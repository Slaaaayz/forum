...
func LikedHandler(w http.ResponseWriter, r *http.Request) {
	tmpl, err := template.ParseFiles("./view/liked.html")
	if err != nil {
	...
	rows, err := models.DB.Query("SELECT post.id, post.name, post.post, post.date, post.idtopic, post.image, post.likes FROM post JOIN likes ON post.id = likes.idpost WHERE likes.uid = ?", uid)
	if err != nil {
		panic(err)
	}
	defer rows.Close()
